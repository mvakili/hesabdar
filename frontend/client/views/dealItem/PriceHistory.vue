  ...
  <list @load-data="loadAsyncData">
    ...
    <template slot="table-template" slot-scope="props">
      <b-table-column field="deal.buyerId" label="فروش / خرید" sortable>
        <span v-if="props.row.deal.buyerId == 1">
          خرید
        </span>
        <span v-else>
          فروش
      ...
      <b-table-column field="pricePerOne" label="فی" sortable>
        {{props.row.pricePerOne | currency('', 0)}}
      </b-table-column>
      <b-table-column field="deal.dealer" label="فروشنده / خریدار">
        <span v-if="props.row.deal.buyerId == 1">
          {{props.row.deal.seller.name}}
        </span>
        <span v-else>
          {{props.row.deal.buyer.name}}
        </span>
      </b-table-column>
      <b-table-column field="deal.dealTime" label="زمان" sortable>
        {{props.row.deal.dealTime | moment('HH:mm jYYYY/jMM/jDD')}}
      </b-table-column>
    </template>
  </list>
